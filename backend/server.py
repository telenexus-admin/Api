...
async def receive_evolution_webhook(
    ...
    Evolution API will send messages here when configured.
    """
    try:
        # Parse the webhook payload
        payload = await request.json()
        logger.info(f"Evolution webhook received for {evolution_instance_name}: {payload}")
        
        # Extract event type
        event = payload.get("event", "")
        
        # Only process incoming messages (messages.upsert with fromMe=false)
        if event != "messages.upsert":
            return {"success": True, "message": "Event ignored"}
        
        # Find the instance by evolution_instance_name
        instance = await db.instances.find_one(
            {"evolution_instance_name": evolution_instance_name},
            {"_id": 0}
        )
        ...
        if not instance:
            logger.warning(f"Instance not found for evolution name: {evolution_instance_name}")
            raise HTTPException(status_code=404, detail="Instance not found")
        ...
        for msg in messages:
            ...
            if "conversation" in message_obj:
                ...
                message_text = f"[Image] {message_obj.get('imageMessage', {}).get('caption', '')}"
            elif "videoMessage" in message_obj:
                message_text = f"[Video] {message_obj.get('videoMessage', {}).get('caption', '')}"
            elif "audioMessage" in message_obj:
                message_text = "[Audio Message]"
            elif "documentMessage" in message_obj:
                message_text = f"[Document] {message_obj.get('documentMessage', {}).get('fileName', '')}"
            else:
            ...
            message_doc = {
                ...
                "evolution_message_id": message_id_evolution,
                "created_at": now
            }
            
            await db.messages.insert_one(message_doc)
            logger.info(f"Stored incoming message {message_id} from {phone_number}")
            
            # Forward to Botpress if configured and active
            if instance.get("instance_type") == "botpress":
                background_tasks.add_task(
                    forward_to_botpress,
                    instance,
                    phone_number,
            ...
            # Trigger user webhooks in background
            background_tasks.add_task(
                trigger_webhooks,
                instance["id"],
                "message.received",
